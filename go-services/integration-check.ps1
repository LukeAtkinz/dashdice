# DashDice Matchmaking System Integration Check & Fix
...
function Test-DockerServices {
    ...
    try {
        $containers = docker ps --format "{{.Names}},{{.Status}}" | Where-Object { $_ -match "dashdice" }
        ...
        foreach($container in $containers) {
            ...
            if($parts[0] -and $parts[1] -match "Up") {
                $runningServices += $parts[0]
                Write-IntegrationStatus "Container $($parts[0]) is running" "SUCCESS"
            }
        }
