...
export class GameInvitationService {
  ...
  private static async createGameSession(invitation: GameInvitation): Promise<string> {
    try {
      ...
      console.error('❌ GameInvitationService: Error creating game session via Go services:', error);
      
      // Fallback to original Firebase method if Go services fail
      console.log('🔄 GameInvitationService: Falling back to Firebase method');
  ...
  private static async createGameSessionFirebase(invitation: GameInvitation): Promise<string> {
    try {
      console.log('🎮 Creating friend game session via Firebase fallback for invitation:', invitation);
      ...
      console.log('✅ Friend game session created via Firebase fallback:', gameId);
      ...
      console.error('❌ Error creating friend game session via Firebase:', error);
  ...
  static startInvitationCleanupTimer(): void {
    ...
    if (typeof window === 'undefined') {
      console.warn('⚠️ GameInvitationService: Cleanup timer skipped (SSR environment)');
      return;
    }
