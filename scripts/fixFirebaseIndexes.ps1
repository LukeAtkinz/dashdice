...
switch ($choice) {
    ...
    "2" {
        ...
        $urls = @(
            "https://console.firebase.google.com/v1/r/project/dashdice-d1b86/firestore/indexes?create_composite=ClVwcm9qZWN0cy9kYXNoZGljZS1kMWI4Ni9kYXRhYmFzZXMvKGRlZmF1bHQpL2NvbGxlY3Rpb25Hcm91cHMvZnJpZW5kUmVxdWVzdHMvaW5kZXhlcy9fEAEaCgoGc3RhdHVzEAEaDAoIdG9Vc2VySWQQARoNCgljcmVhdGVkQXQQAhoMCghfX25hbWVfXxAC",
        ...
        Write-Host "⏱️  Index creation takes 1-2 minutes per index." -ForegroundColor Yellow
    }
    
    "3" {
        Write-Host "⚠️  Temporarily disabling problematic services..." -ForegroundColor Yellow
        Write-Host "This will disable friends and achievements features until indexes are created." -ForegroundColor Red
        Write-Host "Game modes will continue working with default data." -ForegroundColor Green
        
        # This would involve commenting out or disabling the problematic queries
        Write-Host "✅ Services disabled. Create indexes when ready and re-enable." -ForegroundColor Green
    }
